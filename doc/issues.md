...
### P1 12: Sony MakerNote embedded-JPEG offsets/lengths are validated
- **Status:** Done and verified against local RawParserKit.
- **Verification:** RawParserKit validates Sony embedded-JPEG offsets and lengths before returning locations and before reading data. Invalid non-positive, overflowing, out-of-file, and oversized ranges are rejected. Sony slow-path full-file retries now use file-size-aware memory-mapped data instead of eager `read(upToCount: Int.max)`.
- **Tests:** `swift test` passed in `/Users/thomas/GitHub/RawCull/RawParserKit`, including malformed Sony embedded-JPEG range coverage.
- **Note:** RawCull is temporarily wired to local package `/Users/thomas/GitHub/RawCull/RawParserKit` for verification. The HTTPS package update/version bump is intentionally deferred.
...
### P2 27: Raw diagnostics no longer block MainActor on whole-RAW reads
- **Status:** Done and verified against local RawParserKit.
- **Verification:** RawCull now starts RAW diagnostics generation off-main and publishes the finished `RawDiagnosticsPresentation` back on `MainActor`. RawParserKit's Sony diagnostics slow paths now use memory-mapped full-file data rather than eager whole-file reads.
- **Tests:** `swift test` passed in local RawParserKit, and `make test-smoke` passed in RawCull with the current local package wiring.

## Recommended Queue

### 1. Issue 4 — Off-main AppKit drawing in thumbnail downscaling
- **Urgency:** High.
...
### 2. Issue 7 — Catalog switches can leave stale catalog state visible
- **Urgency:** High.
...
### 3. Issue 20 — Copy completion UI can report rsync failures as success
- **Urgency:** High.
...
### 4. Issue 19 — Copy sheet can get stuck in "Copying..." on startup failure
...
### 5. Issue 6 — Thumbnail discovery does not acquire security-scoped access
...
### 6. Issue 5 — Disk caches never invalidate on source file replacement
- **Urgency:** Medium-high.
...
### 7. Issue 8 — Async sort/search can overwrite newer UI state
- **Urgency:** Medium-high.
...
### 8. Issue 9 — Similarity ranking has no stale-result protection
- **Urgency:** Medium-high.
...
### 9. Issue 10 — Burst undo cannot restore "unrated" state
- **Urgency:** Medium-high.
...
### 10. Issue 11 — `asyncgetsettings()` races initial JSON loading
- **Urgency:** Medium.
...
### 11. Issue 14 — Grid rating filter is disconnected from app-wide filter
- **Urgency:** Medium.
...
### 12. Issue 15 — Command-click deselection leaves stale primary selection
- **Urgency:** Medium.
...
### 13. Issue 16 — Comparison focus-point lookup collides on duplicate basenames
- **Urgency:** Medium.
...
### 14. Issue 17 — Histogram initial load can crash
...
### 15. Issue 18 — Zoom buttons desync pinch gesture state
- **Urgency:** Medium.
...
### 16. Issue 21 — Cmd-K abort only works in loupe mode
- **Urgency:** Medium.
...
### 17. Issue 22 — Thumbnail settings persist partially
- **Urgency:** Low-medium.
...
### 18. Issue 23 — `ThumbnailLoader` ignores requested size and freezes settings
- **Urgency:** Low-medium.
...
### 19. Issue 24 — Missing in-flight request coalescing duplicates thumbnail work
- **Urgency:** Low-medium.
...
### 20. Issue 25 — Saved-files JSON loads synchronously on MainActor
- **Urgency:** Low-medium.
...
### 21. Issue 26 — JPG extraction/warm-cache progress callbacks are not identity-gated
- **Urgency:** Low-medium.
...
### 22. Issue 28 — Sharpness cancellation does not stop in-flight Vision/Core Image work promptly
...
### 23. Issue 29 — Shared thumbnail view can apply stale async results
...
### 24. Issue 30 — Comparison grid refreshes can race
- **Urgency:** Low-medium.
...
### 25. Issue 31 — Histogram path does not reach last bin's full width
- **Urgency:** Low.
...
### 26. Issue 32 — Rsync details view drops all rows for small outputs
- **Urgency:** Low.
...
- Run `make test-full` after closing the remaining high-impact P1 queue, especially issues 4, 7, 19, and 20.
