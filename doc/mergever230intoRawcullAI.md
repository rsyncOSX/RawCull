...
- `version-2.3.0` is two commits ahead of the common base.
...
- Add the PhotoAnalysisKit `1.0.1` package reference and product.
...
- PhotoAnalysisKit `1.0.1`, revision `50f198aae61c87dd1b001709a2e90f16365184c1`.
...
- Sharpness algorithm version `4`.
- `FocusDetectorConfig.stableScoringEnergyMultiplier` from the PhotoAnalysisKit-backed configuration.

The sharpness signature change intentionally invalidates old sharpness results. Because sharpness and similarity are stored in the same burst snapshot, the first run after the merge may rebuild the complete snapshot, including similarity artifacts. This is expected cache invalidation, not data corruption.
