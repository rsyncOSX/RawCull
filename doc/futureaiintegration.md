...
| --- | --- | --- | --- |
| 1. Platform and composition boundary | Implemented baseline; verified | The app and test targets now require macOS 27 and use Swift 6. PhotoAIKit is pinned and its CLIP, SAM 3, contracts, storage, workflows, and Vision products are linked. RawCull has a single `RawCullAIIntegration` composition root, canonical RawCull-owned paths, model-resource managers, typed capability state, mask memory/disk stores, a repository, segmentation service, selector, and a narrow `RawCullAISettingsModel`. | The source-controlled mask worker and model-download flow do not exist. The Settings download and saved-data deletion controls are placeholders. |
| 2. Similarity | In progress; Vision migration implemented and verified | Vision feature-print generation and distance semantics now come from PhotoAIKit behind `RawCullSimilarityServicing`. RawCull retains RAW/preview decoding, bounded indexing, ranking policy, progress, cancellation/supersession protection, and the small subject-label penalty. Similarity artifacts are descriptor-complete and validated by backend, model fingerprint, representation/configuration versions, and source fingerprint. Burst persistence moved to schema 5 with a backend-aware signature, and legacy schema-4 data is rejected before rebuild. | CLIP is instantiated when valid resources exist but is not selected by the feature model. The Settings toggle is deliberately a no-op. Preference persistence and whole-batch CLIP-to-Vision fallback are not implemented. |
...
- `make test-smoke` succeeds, including the new PhotoAIKit checks for descriptor-complete Vision artifacts, RawCull ranking-policy parity, and schema-4 invalidation/schema-5 reload.
- A focused `RawCullAIIntegrationTests` run succeeds for canonical paths, the complete Phase 1 capability surface, persisted Vision evidence scanning, and the deliberate no-op safety of placeholder controls.
...
### Current quality observations before the next feature slice

1. **Move repeated model validation off the main actor or cache it.** `RawCullApp` constructs the `@MainActor` composition root synchronously. With installed resources, provider creation and later capability refreshes can repeat PhotoAIKit model-bundle validation and fingerprinting. At the pinned dependency revision, a manifest cryptographic fingerprint can require hashing the full model asset. Measure cold launch and Settings refresh with real model bundles, then ensure this work is cached and does not block the UI before enabling CLIP or SAM.
2. **Finish cancellation propagation for background helpers.** Similarity indexing has an owned task, cooperative cancellation, and generation checks. In contrast, saved-evidence scanning and similarity ranking currently await new unstructured `Task { @concurrent in ... }` instances. Cancelling the caller prevents some state commits but does not automatically cancel the inner work. Convert these to a structured or explicitly owned cancellation path before the workloads grow.
...
#### What each quality observation means and what a fix requires

These are quality gates for activating the next AI features, not six known user-data failures. They can be designed and reviewed without changing the deployment target, but their eventual code changes must be built, tested, and profiled with the macOS 27/Xcode 27 toolchain used by this branch. The work described below is intentionally not part of this documentation update.

1. **Move repeated model validation off the main actor or cache it.**

   **What is happening now:** `RawCullApp.init()` creates `RawCullAIIntegration` synchronously. Because the application and composition root are main-actor isolated, all provider setup performed during that initialization is on the UI's actor. Creating the SAM 3 and CLIP providers asks PhotoAIKit to locate and validate candidate model bundles. A later Settings refresh calls `integration.capabilities()`, which asks the resource managers to inspect those bundles again. With no installed models this is usually cheap, but a valid manifest can require a cryptographic fingerprint of the full model asset. Large model files could therefore make cold launch or “Check Again” appear to freeze.

   **What the fix involves:** First measure cold launch and repeated capability refresh with real SAM 3 and CLIP bundles, recording the time spent locating resources, validating manifests, and hashing assets. Then choose an ownership boundary for that work. One option is an asynchronous resource-validation service that runs outside the main actor and publishes a typed result back to `RawCullAISettingsModel`. Another is a validation cache, preferably in PhotoAIKit if the behavior is reusable, keyed by enough file metadata and manifest identity to invalidate when a model changes. The UI should start with a lightweight `checking` or last-known capability state instead of waiting synchronously for validation. Provider creation must consume the same validated result so it does not immediately repeat the work. Completion requires tests for cache reuse and invalidation, plus launch/refresh profiling showing that an unchanged bundle is not fully hashed repeatedly and that the main actor remains responsive.

2. **Finish cancellation propagation for background helpers.**

   **What is happening now:** The main similarity-indexing operation owns its `Task`, cancels the previous task, checks a generation number, and cooperatively stops work. Two smaller paths do less. `RawCullAISettingsModel.refresh()` creates a new unstructured concurrent task for the saved-evidence scan, and `SimilarityScoringModel.rankSimilar(...)` does the same for distance calculation. If the caller is cancelled, it stops the final state update after awaiting the inner task, but cancellation is not automatically forwarded into that independently created task. The scan or distance loop can continue consuming disk or CPU even though its result will be discarded.

   **What the fix involves:** Prefer making the scanners and computation helpers asynchronous, `@concurrent` operations that are awaited directly in the caller's existing task. That preserves the caller's cancellation context without introducing another task lifetime. If an independently owned task is genuinely needed for supersession, store it, cancel it explicitly from a cancellation handler and when a replacement starts, and clear it only if it is still the current task. Long scans and distance loops must check cancellation at natural boundaries; merely checking before the final UI assignment is insufficient. Keep generation checks as protection against stale results because cancellation is cooperative and can race with completion. Completion requires cancellation tests that start sufficiently large work, cancel or supersede it, verify that the helper exits early, and confirm that neither stale state nor a stuck loading flag is left behind.
