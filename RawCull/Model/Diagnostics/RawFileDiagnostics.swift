...
enum RawFileDiagnostics {
    static func issues(for file: FileItem) -> [RawFileDiagnosticIssue] {
        var issues: [RawFileDiagnosticIssue] = []
        ...
        if FileManager.default.fileExists(atPath: file.url.path) {
            if !FileManager.default.isReadableFile(atPath: file.url.path) {
                issues.append(issue(
                    ...
                    recoveryHint: "Check Finder permissions or security-scoped access.",
                ))
            }
        } else {
            issues.append(issue(
                ...
                recoveryHint: "Confirm the source catalog still contains this RAW file.",
            ))
            return issues
        }
        ...
        guard let format = RawFormatRegistry.format(for: file.url) else {
            issues.append(issue(
                ...
                category: .catalog,
                severity: .blocking,
                message: "Unsupported RAW extension.",
                recoveryHint: "Use a supported Sony ARW or Nikon NEF file.",
            ))
            return issues
        }

        if file.exifData == nil {
            issues.append(issue(
                file: file,
                category: .catalog,
                severity: .warning,
                message: "No scanned EXIF metadata stored on selected file.",
                recoveryHint: "Rescan the catalog if metadata should be available.",
            ))
        }

        let sourceOptions = [kCGImageSourceShouldCache: false] as CFDictionary
        if CGImageSourceCreateWithURL(file.url as CFURL, sourceOptions) == nil {
            issues.append(issue(
                file: file,
                category: .parser,
                severity: .blocking,
                message: "ImageIO could not create image source.",
                recoveryHint: "Open diagnostics to inspect the RAW container.",
            ))
        }

        switch format {
        case is SonyRawFormat.Type:
            let jpeg = SonyMakerNoteParser.embeddedJPEGLocationsDiagnostics(from: file.url)
            if let failure = jpeg.failure {
                issues.append(issue(
                    file: file,
                    category: .parser,
                    severity: .warning,
                    message: "Sony embedded JPEG parser: \(failure)",
                    recoveryHint: "Open diagnostics to inspect the MakerNote trace.",
                ))
            }

            let focus = SonyMakerNoteParser.focusLocationDiagnostics(from: file.url)
            if focus.value == nil {
                issues.append(issue(
                    file: file,
                    category: .parser,
                    severity: .warning,
                    message: "Sony AF focus parser: \(focus.failure ?? "unknown failure")",
                    recoveryHint: "Open diagnostics to inspect the AF focus trace.",
                ))
            }

        case is NikonRawFormat.Type:
            let jpeg = NikonMakerNoteParser.embeddedJPEGLocationsDiagnostics(from: file.url)
            if let failure = jpeg.failure {
                issues.append(issue(
                    file: file,
                    category: .parser,
                    severity: .warning,
                    message: "Nikon embedded JPEG parser: \(failure)",
                    recoveryHint: "Open diagnostics to inspect the MakerNote trace.",
                ))
            }

            let focus = NikonMakerNoteParser.focusLocationDiagnostics(from: file.url)
            if focus.value == nil {
                issues.append(issue(
                    file: file,
                    category: .parser,
                    severity: .warning,
                    message: "Nikon AF focus parser: \(focus.failure ?? "unknown failure")",
                    recoveryHint: "Open diagnostics to inspect the AF focus trace.",
                ))
            }
        ...
        default:
            issues.append(issue(
                ...
                category: .parser,
                severity: .warning,
                message: "No embedded JPEG locator registered for \(formatName(format)).",
                recoveryHint: "Open diagnostics to inspect format support.",
            ))
        }
