...
DataComp records its published v3 archive. The disabled OpenAI CLIP entry retains its v2 archive; it is not included in v3. EfficientSAM and SAM 3
...
SAM 3 selection, loading, and downloads are disabled while its checked-in provenance remains blocked. Its archive metadata in the prepared catalog does not constitute distribution approval.
