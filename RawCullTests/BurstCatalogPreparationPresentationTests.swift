...
struct BurstCatalogPreparationPresentationTests {
    @Test
    func `similarity indexing is the first visible stage`() throws {
        let presentation = makePresentation(
            similarityIndexedCount: 200,
            isIndexing: true,
        ...
        #expect(presentation.activeStage == .similarityIndex)
        #expect(presentation.estimatedSeconds == 80)
        #expect(
            presentation.overallCompletionFraction
                == (Double(200) / Double(905)) / 3,
        )
        #expect(try status(for: .similarityIndex, in: presentation) == .running(
            activity: .indexing,
            completed: 200,
            total: 905,
        ))
    ...
    func `sharpness calibration is visible even before scoring begins`() throws {
        ...
        #expect(try status(for: .similarityIndex, in: presentation) == .complete(
            completed: 905,
            total: 905,
        ))
    ...
    private func makePresentation(
        ...
        BurstCatalogPreparationPresentation(
            ...
            fileCount: 905,
            similarityIndexedCount: similarityIndexedCount,
            similarityCatalogCount: 905,
